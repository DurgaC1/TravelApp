...
const SignIn: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Construct user profile (as expected by your backend)
      const profile = {
        firstName: user.displayName?.split(" ")[0] || "",
        lastName: user.displayName?.split(" ").slice(1).join(" ") || "",
        email: user.email || "",
        mobile: "", // Google doesn't give mobile
        createdAt: new Date().toISOString(),
      };

      // Send to backend to create user if not exists
      try {
        const res = await fetch("http://localhost:3002/api/users/profile", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`, // assuming you use Firebase token in your backend
          },
        ...
        if (!res.ok) {
          ...
          console.warn("Backend profile creation failed:", errorText);
          // Continue anyway since auth succeeded
        }
      } catch (backendError) {
        console.warn("Error connecting to backend:", backendError);
      }
        ...
        {isAuthenticated ? (
          ...
          <>
            ...
            <div className="flex justify-center">
              <Button
                variant="outline"
                className="flex items-center justify-center border-purple-300 text-purple-500 hover:bg-purple-200"
  ...
  <Button
    variant="outline"
    className="flex items-center justify-center border-purple-300 text-purple-500 hover:bg-purple-200 ml-4"
    disabled
  >
    <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
      <path
